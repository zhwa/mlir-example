...
namespace mlir {
  ...
  mlir::OwningOpRef<mlir::ModuleOp> createGemmModuleTensor(mlir::MLIRContext &context);
...
PYBIND11_MODULE(ch4_tensor_bufferization, m) {
  m.doc() = "MLIR-powered matrix multiplication accelerator (Chapter 4: Tensor-bufferization)";
  ...
  m.def("test_tensor_ir", []() -> std::string {
    mlir::MLIRContext context;
    context.getOrLoadDialect<mlir::func::FuncDialect>();
    context.getOrLoadDialect<mlir::arith::ArithDialect>();
    context.getOrLoadDialect<mlir::linalg::LinalgDialect>();
    ...
    auto module = createGemmModuleTensor(context);
    if (!module) {
      return "ERROR: Failed to create tensor-based MLIR module";
    }
    ...
    os.flush();
    return result;
  }, "Test tensor-based MLIR IR generation and return the generated MLIR code");
}