...
# The unified build system places modules in: ../build/x64-release/ch.1.Fixed-size/
build_paths = [
    '../build/x64-release/ch.1.Fixed-size',
    '../build/x64-debug/ch.1.Fixed-size',
    'build/x64-release/ch.1.Fixed-size',  # If running from root
    'build/x64-debug/ch.1.Fixed-size',
]

for path in build_paths:
    if os.path.exists(path) and any(f.startswith('ch1_fixed_size') for f in os.listdir(path) 
                                     if not os.path.isdir(os.path.join(path, f))):
        sys.path.insert(0, path)
        print(f"Using build directory: {path}")
        break
else:
    print("ERROR: Could not find ch1_fixed_size module!")
