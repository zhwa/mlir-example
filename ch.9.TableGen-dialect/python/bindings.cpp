...
public:
    ...
    py::array_t<float> numpy() {
        if (!is_computed_) {
            throw std::runtime_error("Tensor not computed yet. Call compile() first.");
        }
        return data_;
    }
...
class GraphCompiler {
public:
    static py::array_t<float> compile(std::shared_ptr<Tensor> output) {
...
PYBIND11_MODULE(ch9, m) {
    ...
    m.def("compile", &GraphCompiler::compile, "Compile and execute tensor graph");
