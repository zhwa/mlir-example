...
This script tests both correctness and performance:
  1. Correctness tests - Compare against NumPy
  2. Performance benchmark - Measure JIT caching speedup
  3. Shape flexibility - Test various matrix dimensions
"""
...
# The unified build system places modules in: ../build/x64-release/ch.3.JIT-caching/
build_paths = [
    '../build/x64-release/ch.3.JIT-caching',
    '../build/x64-debug/ch.3.JIT-caching',
    'build/x64-release/ch.3.JIT-caching',  # If running from root
    'build/x64-debug/ch.3.JIT-caching',
]

for path in build_paths:
    if os.path.exists(path) and any(f.startswith('ch3_jit_caching') for f in os.listdir(path) 
                                     if not os.path.isdir(os.path.join(path, f))):
        sys.path.insert(0, path)
        print(f"Using build directory: {path}")
        break
else:
    print("ERROR: Could not find ch3_jit_caching module!")
